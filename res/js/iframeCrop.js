var containerWidth = 432, containerHeight = 294; //左右图片容器的尺寸
var extArr = ["gif"];
...
$(document).ready(function () {
    if ($('#url').val()) {
        if (isURL((url))) {
            //loadImage('http://s1.dwstatic.com/group1/M00/7B/C6/8de53f774764b8aa2460caf2850e9449.gif');
        }
    }
    //添加水印
    ...
    //加载默认图片
	(function(){
		var _search = window.location.search.replace("?src=","");
		if(_search){
			$("#url").val(_search);
    ...
    $(document).on("submit", "#download_form", function (e) {
        e.preventDefault();
        $("#download_form input:submit").attr("disabled", true);
        //showInfoTip('<img src="res/images/loading.gif" alt="Loading..." />');
        $.ajax({
            type: "POST",
            url: $(this).attr("action"),
            data: $(this).serialize(),
            success: function (e) {
            	postMessage("http://106.75.12.84/gifCrop/uploads/"+e.message.fileName);
                ...
                //window.location.href = 'crop/responseFile?fileName=' + e.message.fileName;
                //var imgHtml = '<img id="newImg" name="newImg" src="uploads/' + e.message.fileName + '?v='+new Date()+'" style="width:' + e.message.w + 'px;height:' + e.message.h + 'px"/>';
                //$("#error").html(imgHtml), $("#error").fadeTo(150, 1), $("#download_form input:submit").attr("disabled", false);
            },
            beforeSend: function () {
                disableBtn();
                $('.download-btn').hide();
                addSpinner($('.download-btn-inner'));
               /* $('.spinner').css("margin-top", 7+'px');//loading 图标居中*/
            },
            complete: function () {
        ...
        _hmt.push(['_trackEvent', 'gif裁剪', 'click', '裁剪并保存']);
    });//http://106.75.12.84/gifCrop/uploads/default_small.gif
    ...
    $('.watermark-upload-icon').on('click', function () {
        _hmt.push(['_trackEvent', 'gif裁剪', 'click', '本地上传']);
    });
});
...
function make_btn_clk() {
    ...
    _hmt.push(['_trackEvent', 'gif裁剪', 'click', '开始制作']);
}
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            if (data.status) {
                ...
                //非gif 图片预加载成功， server 判断非 gif 图片后 ：重新加载默认图片
                loadImage('uploads/default.gif', true);
                $('#srcFileName').val('default.gif');
            }
            _hmt.push(['_trackEvent', 'gif裁剪', 'ajax', 'URL正常读取']);
        },
        error: function (status, err) {
            showInfoTip('上传失败，请刷新页面重试');
            //$('.make-btn-txt').show();
            $('.make-btn').addClass('make-btn-warn');
            _hmt.push(['_trackEvent', 'gif裁剪', 'ajax', 'URL读取失败——"链接不正确"']);
        },
        beforeSend: function () {
            $('.download-btn').hide();
            addSpinner($('.download-btn-inner'));
...
function loadImage(url, isInit,isResetWatermark) {
    ...
    img.onload = function () {
        ...
        if (!isInit) {//初始化图片时无需显示成功状态
            $('.make-btn').addClass('make-btn-success');
            $(".watermark-outer").addClass("_success");
...
function onSelectEnd(img, selection) {
    ...
    _hmt.push(['_trackEvent', 'gif裁剪', 'click', '调整尺寸']);
}
...
function imgZoom(id) {
    ...
    var maxWidth = 432, maxHeight = 294;
...
function showInfoTip(info) {
    ...
    hideBtnTip();
    $('.make-btn').addClass('make-btn-warn');
    $(".watermark-outer").remove("_success").addClass("_error");
...
function hideBtnTip() {
    var btn = $('.make-btn');
    if (btn.hasClass("make-btn-success")) {
        $(".watermark-outer").removeClass("_success");
        btn.removeClass("make-btn-success")
    };
    if (btn.hasClass("make-btn-warn")) {
        $(".watermark-outer").removeClass("_error");
        btn.removeClass("make-btn-warn")
    };
}
