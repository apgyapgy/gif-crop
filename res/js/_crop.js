...
$(document).ready(function () {
    ...
    $(document).on("submit", "#download_form", function (e) {
        ...
        _hmt.push(['_trackEvent', '对外_gif裁剪', 'click', '裁剪并保存']);
    });
    ...
    $('.watermark-upload-icon').on('click', function () {
        _hmt.push(['_trackEvent', '对外_gif裁剪', 'click', '本地上传']);
    });
});
///
function make_btn_clk() {
    ...
    _hmt.push(['_trackEvent', '对外_gif裁剪', 'click', '开始制作']);
}
...
function onSelectEnd(img, selection) {
    ...
    _hmt.push(['_trackEvent', '对外_gif裁剪', 'click', '调整尺寸']);
}
