...
var args,gifId,extensionId,channel="";
var _selfFlag = false;//判断用户是否自己上传图片
$(document).ready(function () {
	...
	sendExtensionMessage(extensionId, 'getCachedGif', {gifId: gifId}, function(res){
		...
		isUploadBase64 = true;
		downloadRemoteImage(res,function(){
			_selfFlag = false;
		});
   		return true;
	});
    ...
    $(document).on("submit", "#download_form", function (e) {
        ...
        $.ajax({
            ...
            success: function (e) {
            	...
            	$.ajax({
            		type:'GET',
            		url:"../crop/imageToBase64?fileName="+e.message.fileName,
            		success:function(re){
        ...
        _hmt.push(['_trackEvent', '对外_gif裁剪-'+channel, 'click', '裁剪并保存']);
    });
    ...
    $('.watermark-upload-icon').on('click', function () {
        _hmt.push(['_trackEvent', '对外_gif裁剪-'+channel, 'click', '本地上传']);
    });
});
///
function make_btn_clk() {
    ...
    if (url) {
        if (!isURL((url))) {
            ...
            if (!base64regex.test(url)) { //是否是 base64 字符串
                ...
                showInfoTip('请输入正确的网页图片地址或Base64图片字符');
    ...
    _hmt.push(['_trackEvent', '对外_gif裁剪-'+channel, 'click', '开始制作']);
}
...
function downloadRemoteImage(_base64Url,fn) {
	if(_base64Url){
		var _data = {
			'url':_base64Url
		}
	...
	if(fn){
		...
		_selfFlag = true;
		$("#download_form .download-btn").html("裁剪并下载");
	}
    $.ajax({
        type: "POST",
...
//      data: {'url': url}, //定义全局 url ,因为上传 base64 图片时，make_btn_clk 中会过滤掉头部信息 : data:image/gif;base64  , 此时 base64 会很长，要放在 post 里面
        ...
        success: function (data) {//console.log("success");
            ...
            _hmt.push(['_trackEvent', '对外_gif裁剪-'+channel, 'ajax', 'URL正常读取']);
        },
        error: function (status, err) {//console.log("error");
            showInfoTip('上传失败，请刷新页面重试');
            $('.make-btn').addClass('make-btn-warn');
            _hmt.push(['_trackEvent', '对外_gif裁剪-'+channel, 'ajax', 'URL读取失败——"链接不正确"']);
        },
        beforeSend: function () {
            $('.download-btn').hide();
            addSpinner($('.download-btn-inner'));
...
function onSelectEnd(img, selection) {
    ...
    _hmt.push(['_trackEvent', '对外_gif裁剪-'+channel, 'click', '调整尺寸']);
}
